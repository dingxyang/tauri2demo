...
if [[ -x "$SDKMANAGER" ]]; then
  ...
  warn "未找到 sdkmanager（路径：$SDKMANAGER），建议安装 cmdline-tools（可选）"
fi
...
# ─── 8. Device / Emulator check (dev only) ────────────────────────────────────
if [[ "$COMMAND" == "dev" ]]; then
  echo -e "${CYAN}[8/8] Android 设备或模拟器${RESET}"
  if [[ -x "$ADB" ]]; then
  ...
  echo -e "${CYAN}[8/8] 构建模式，跳过设备检查${RESET}"
  ok "构建模式无需连接设备"
fi
